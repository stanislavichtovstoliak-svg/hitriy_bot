...
def work_cmd(message):
    ...
    if user['last_work']:
        ...
        if diff < 600:
            ...
            m = rem // 60
            s = rem % 60
            bot.send_message(message.chat.id, f"⏰ Отдыхай *{m} мин {s} сек*", parse_mode='Markdown')
            return
...
def roulette_bet_amount(message):
    ...
    try:
        ...
        if bet < 10:
            bot.send_message(message.chat.id, "❌ Минимум 10!", parse_mode='Markdown')
            return
    except:
    ...
    if user['money'] < bet:
        bot.send_message(message.chat.id, f"❌ Не хватает {bet}!", parse_mode='Markdown')
        del roulette_waiting[uid]
        return
