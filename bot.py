...
def profile_cmd(m):
    try:
        ...
        msg += f"🏆 Достижений получено: {len(u.get('achievements', []))}\n"
...
def slots_play(m):
    ...
    anim = bot.send_message(m.chat.id, "🎰 СЛОТЫ 🎰\n\n🌀 | 🌀 | 🌀\n🌀 | 🌀 | 🌀\n🌀 | 🌀 | 🌀\n\nКРУТИМ...")
    
    for step in range(6):
        time.sleep(1)
        temp = [[random.choice(SLOT_SYMBOLS) for _ in range(3)] for _ in range(3)]
